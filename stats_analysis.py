...
def wilcoxon_signed_rank(x, y):
    ...
    W = min(Wp, Wm)
    mu = n * (n + 1) / 4.0
    _, _tc = np.unique(np.abs(d), return_counts=True)          # tie-adjusted variance
    _tie = float(np.sum(_tc ** 3 - _tc))
    var = (n * (n + 1) * (2 * n + 1) - _tie / 2.0) / 24.0
...
def analyse(M, methods, tag, hh="HH-UCB"):
    ...
    _T = 0.0                                                     # Friedman tie correction
    for _i in range(N):
        _, _c = np.unique(ranks[_i], return_counts=True); _T += float(np.sum(_c ** 3 - _c))
    _corr = 1.0 - _T / (N * k * (k * k - 1))
    if _corr > 0: chi2 = chi2 / _corr
    ...
    for j, m in enumerate(methods):
        if m == hh: continue
        _, p = wilcoxon_signed_rank(ranks[:, hi], ranks[:, j]); praw[m] = p  # scale-invariant: compare within-problem ranks
    padj = holm(praw)
