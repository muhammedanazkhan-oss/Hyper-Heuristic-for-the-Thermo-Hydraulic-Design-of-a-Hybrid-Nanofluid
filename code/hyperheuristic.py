...
class HyperHeuristic:
    ...
    def _llh(self, k):
        ...
        if k == 3:
            ...
            c[rng.choice(self.d)] = self.best_x[rng.choice(self.d)]
            return _clip(c, self.lo, self.hi)
        if k in (4, 5):
...
def cma_es(problem, budget, rng, sigma0=0.3):
    ...
    while nev < budget:
        ...
        f = np.array([p.eval(x) for x in X]); nev += lam
        order = np.argsort(f)
        for e, fi in enumerate(f[order]):
            ix = nev - lam + e
            if fi < best_f:
                best_f = float(fi); best_x = X[order[e]].copy()
            if ix < budget:
                hist[ix] = best_f
