...
```
Hyper-Heuristic-ETC-Nanofluid-v1.0.3/
├── README.md                 this file
...
├── REPRODUCIBILITY.md        scope of bit-exact reproduction; pinned stack
├── Dockerfile                pinned environment for bit-exact reproduction
├── set_doi.sh                inserts the reserved Zenodo DOI into the metadata files
├── requirements.txt          Python dependencies
├── data/
│   ├── ETC_Hybrid_Nanofluid_Sweep.xlsx   full-factorial dataset (sheet "Simulation Data", 54,432 runs)
│   └── DATASET_NOTE.md        where the dataset lives / how it carries forward
└── code/                     working directory: run all scripts from here
    ...
    ├── verify_reproduction.py    fast PASS/FAIL reproduction check (see REPRODUCIBILITY.md)
    ├── append_revision.py        supplementary analyses (blocked CV, closure
    │                              sensitivity, conditional-optimum stability, encoding
    │                              invariance, top-region PEC ranking); revision_results.json
    │                              is also shipped precomputed
...
# supplementary analyses (revision_results.json is shipped precomputed;
# this regenerates the appended entries)                       -> revision_results.json
python append_revision.py
...
# fast reproduction check (analysis layer + deterministic controls + rank invariance)
python verify_reproduction.py
```

`run_experiments.py` is the only long step (it executes 12 problems x 16 methods x 30 seeds). It is **resumable** and **seeded from a fixed master seed** (see `SEED_BASE` in the script) so results are deterministic within a fixed numerical stack. On Windows it automatically uses the `spawn` multiprocessing start method. Because `results.csv`, `conv/`, `traces/` and the surrogate models are included, the statistics, tables and figures can be regenerated immediately without rerunning it, and they reproduce bit-for-bit on any platform.
...
**See `REPRODUCIBILITY.md` for the full statement and a pinned container recipe.** In brief:

- The **analysis layer is deterministic on every platform**: `stats_analysis.py`, `compile_tables.py` and `make_figures.py` regenerate the statistics, tables and figures bit-for-bit from the shipped `results.csv`, `conv/` and `traces/`. This is the layer that produces every number in the article.
- When `results.csv` is **regenerated from scratch**, ten of the twelve methods (including the deterministic controls DE, HH-Random and the standalone LNS operator) reproduce bit-for-bit anywhere. Only **HH-UCB** (which evaluates a libm `log`) and **CMA-ES** (which calls the LAPACK `numpy.linalg.eigh`) are stack-dependent: these two primitives are not bit-portable across BLAS/libm builds and CPU microkernels, so on a chaotic multimodal landscape their per-seed values can differ in the last decimals across machines. Pinning the NumPy version alone is not sufficient; use the `Dockerfile` for a fixed stack. `verify_reproduction.py` checks all three layers.
- This residual divergence changes **no reported conclusion**: the statistics operate on within-problem ranks, and substituting an independent stack's divergent values leaves every Friedman rank, the Friedman statistic and every Holm-Wilcoxon decision identical, and the collector optima unchanged.
