...
def fig_convergence():
    ...
    for ax,(p,make,lab,yl) in zip(axes[2:],[("ETC-Eff-op1",lambda:TEff(40,1200,25,Wp_cap=0.5),"thermal efficiency",(0.40,0.78)),
        ...
        for m in show:
            mu,sd=obj_curve(make,m,700,seeds=30); x=np.arange(1,len(mu)+1)
            ax.plot(x,mu,color=MCOL.get(m,'#444'),lw=2.2,label=m.replace("LLH:",""))
            ax.fill_between(x,mu-sd/np.sqrt(30)*1.96,mu+sd/np.sqrt(30)*1.96,color=MCOL.get(m,'#444'),alpha=0.16,lw=0)
...
def fig_selection_and_credit():
    ...
    ax.set_xlabel("operator invocations"); ax.set_ylabel("selection fraction (sliding window)")
    ...
    ax.set_xlabel("operator invocations"); ax.set_ylabel("running mean reward (new-best rate)")
...
def fig_stats():
    ...
    R=np.array([SA._rank_tol(M[r]) for r in range(M.shape[0])])   # within-problem ranks (scale-invariant)
    for i in range(k):
        for j in range(k):
            if i!=j: _,pp=SA.wilcoxon_signed_rank(R[:,i],R[:,j]); P[i,j]=pp
...
def main():
    for fn in [fig_convergence, fig_selection_and_credit, fig_boxplots, fig_stats,
               fig_radar, fig_sensitivity, fig_landscape]:
        fn()
